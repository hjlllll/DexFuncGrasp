...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('--idx', type=int, default=0, help="0-20 id of hjl new dataset, means category")
    ...
    experiment_directory = os.path.join(data_root, category, 'split')
    ...
    saved_model_state = torch.load(
        os.path.join(experiment_directory, "network", ws.model_params_subdir, args.checkpoint + ".pth")
    )
    ...
    with open(os.path.join(experiment_directory, "split.json"), "r") as f:
        split = json.load(f)

    npz_filenames = deep_sdf.data.get_instance_filenames(experiment_directory, split)
    ...
    reconstruction_dir = os.path.join(experiment_directory, ws.reconstructions_subdir)
    ...
    latent_code = torch.load(
        os.path.join(experiment_directory, "network", ws.latent_codes_subdir, f"{args.checkpoint}.pth")
    )
    for ii, npz in enumerate(npz_filenames):
        ...
        full_filename = os.path.join(experiment_directory.split('/split')[0], ws.sdf_samples_subdir, npz)
        ...
        if not save_latvec_only:
            start = time.time()
            sdf_scale = loadmat(full_filename)
            scale = sdf_scale['scale']
            with torch.no_grad():
                if os.path.exists(mesh_filename + ".ply"):
                    continue
                deep_sdf.mesh.create_mesh(decoder, latent, mesh_filename, N=256, max_batch=int(2 ** 18),scale=scale)
