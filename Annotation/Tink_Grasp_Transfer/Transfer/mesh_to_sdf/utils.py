...
def scale_to_unit_sphere_dif(mesh):
    if isinstance(mesh, trimesh.Scene):
        mesh = mesh.dump().sum()
    ...
    # scale = (1.03*np.max(distances))
    #from idx 11
    vertices /= (1.1 * np.max(distances))
...
def scale_to_unit_cube(mesh):
    ...
    radius_bbox = np.amax(abs(vertices), axis=0)
    vertices = vertices / np.linalg.norm(radius_bbox) #* 0.5

    return trimesh.Trimesh(vertices=vertices, faces=mesh.faces)


def scale_to_mesh(mesh):
    if isinstance(mesh, trimesh.Scene):
        mesh = mesh.dump().sum()
    ...
    max_norm = 0
    vertices = np.asfarray(mesh.vertices, dtype=np.float32)
    bbox_center = (vertices.min(0) + vertices.max(0)) / 2
    norm = np.linalg.norm(vertices - bbox_center, axis=1).max()
    max_norm = max(max_norm, norm)
    vertices = vertices - bbox_center[None]
