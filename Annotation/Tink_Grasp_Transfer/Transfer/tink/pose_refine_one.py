...
from hand_optimizer_one import GeOptimizer, init_runtime_viz
...
if __name__ == "__main__":
    ...
    for idx, (target_instance, soruce_idx) in enumerate(handle_list):
        ...
        for info_transferred_instance in info_transferred_list:
            ...
            #read source grasps
            # grasp_list = os.listdir(os.path.join(grasp_root, str(arg.idx) + '_' + soruce_idx, 'new'))
            # for each_grasp in grasp_list:
            #     if each_grasp.endswith('.xml') and each_grasp.split(''):
            ...
            angles = a[:, index].clone()
            opt1 = GeOptimizer(
                device=device,
                info=[t, input_rwxyz0, angles],  # 3 + 4 + 18 = 25
                lr=arg.lr,
                n_iter=arg.iters,
                verbose=False,
            ...
            opt2 = GeOptimizer(
                ...
                verbose=False,
                lambda_contact_loss=10.0,  # 单位mm
                lambda_repulsion_loss=8e10,  # 单位m/100
            )
            ...
            optimized_all_tensor = opt2.joint.clone().detach()
            optimized_all = opt2.joint.clone().detach().cpu().numpy()
            optimized_angles = trans18_to_22(opt2.joint[7:].reshape(1, 18).clone().detach()).cpu().numpy().reshape(22)
            new_name = os.path.join(os.path.join(grasp_root, str(arg.idx) + '_' + soruce_idx, 'transferred', target_instance), each_grasp)
            rs_wxyz = np.zeros(4)
            ...
            num = int(each_grasp.split('_')[1].split('.')[0])
