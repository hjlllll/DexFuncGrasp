...
import trimesh
import trimesh.transformations as tra #————————> change to torch


def get_rot_axis_mat(rot_axis_list, theta):
    rot_axis_list = [0.571, 0, 0.821]
    rot_axis = torch.FloatTensor(rot_axis_list).reshape(-1, 3, 1)#.to(self.device).reshape(22, 3, 1)  # [22, 3, 1]
    rot_axis_T = torch.transpose(rot_axis, 1, 2)  # [1, 1, 3]
    nnT = torch.bmm(rot_axis, rot_axis_T)  # [1, 3, 3]
    skew = torch.zeros_like(nnT)#.to(self.device)  # [1, 3, 3]
    skew[:, 0, 1] = -rot_axis[:, 2, 0]
    skew[:, 0, 2] = rot_axis[:, 1, 0]
    skew[:, 1, 0] = rot_axis[:, 2, 0]
    skew[:, 1, 2] = -rot_axis[:, 0, 0]
    skew[:, 2, 0] = -rot_axis[:, 1, 0]
    skew[:, 2, 1] = rot_axis[:, 0, 0]

    # batch_size = theta.size()[0]
    cos_theta = torch.cos(theta).reshape(1, 1, 1).repeat(1, 3, 3).float()  # [batch, 22, 3, 3]
    sin_theta = torch.sin(theta).reshape(1, 1, 1).repeat(1, 3, 3).float()  # [batch, 22, 3, 3]
    I = torch.eye(3)#.to(self.device).reshape(1, 3, 3).repeat(1, 1, 1)  # [batch, 22, 3, 3]
    # nnT = nnT.unsqueeze(0).repeat(1, 1, 1)  # [batch, 22, 3, 3]
    # skew = skew.unsqueeze(0).repeat(1, 1, 1)  # [batch, 22, 3, 3]
    mat = cos_theta * I + (1 - cos_theta) * nnT + sin_theta * skew.float()
    return mat
...
class ShadowGripper():
    ...
    def get_meshes(self, data):
        ...
        #LF 20221205修改了几处错误 旋转以及平移
        lfj4_t = np.array([-0.017-0.017, 0, 0.044-0.023]) * 1000.0  # why?
        lfj4_r = tra.euler_matrix(0.571 * joint[0], 0, 0.821 * joint[0])
        lfj4_r[:3, :3] = get_rot_axis_mat(rot_axis_list = [0.571, 0, 0.821], theta=joint[0])
        ...
        lfj3_t = np.array([-0.017+0.016, 0, 0.044+0.023]) * 1000.0
        ...
        lfj3_r = np.dot(lfj4_r, lfj3_r1)
        lfj3_r = np.dot(lfj3_r, lfj3_r2)
        ...
        lfj2_t = np.array([0, 0, 0.045]) * 1000.0
        ...
        lfj1_t = np.array([0, 0, 0.025]) * 1000.0
        ...
        # RF
        rfj3_t = np.array([-0.011, 0, 0.095])* 1000.0
        ...
        rfj3_r = np.dot(palm_r, rfj3_r1)
        rfj3_r = np.dot(rfj3_r, rfj3_r2)
        ...
        rfj2_t = np.array([0, 0, 0.045])* 1000.0
        ...
        rfj1_t = np.array([0, 0, 0.025])* 1000.0
        ...
        # MF
        mfj3_t = np.array([0.011, 0, 0.099])* 1000.0
        ...
        mfj3_r = np.dot(palm_r, mfj3_r1)
        mfj3_r = np.dot(mfj3_r, mfj3_r2)
        ...
        mfj2_t = np.array([0, 0, 0.045])* 1000.0
        ...
        mfj1_t = np.array([0, 0, 0.025])* 1000.0
        ...
        #FF
        ffj3_t = np.array([0.033 ,0 ,0.095])* 1000.0
        ...
        ffj3_r = np.dot(palm_r, ffj3_r1)
        ffj3_r = np.dot(ffj3_r, ffj3_r2)
        ...
        ffj2_t = np.array([0,0,0.045])* 1000.0
        ...
        ffj1_t = np.array([0,0,0.025])* 1000.0
        ...
        #TH
        thj3_t = np.array([0.034 ,-0.009 ,0.029])* 1000.0
        ...
        thj2_t = np.array([0, 0, 0.038])* 1000.0
        ...
        thj1_t = np.array([0 ,0 ,0.032])* 1000.0
