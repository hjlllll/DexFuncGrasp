    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataset):
            ...
            if total_steps % opt.print_freq == 0:
                ...
                if opt.arch == "vae":
                    loss = [
                        model.loss, model.kl_loss, model.reconstruction_pc_loss,
                        model.confidence_loss2,
                        model.angle_loss,model.r_loss,model.t_loss
                    ]
                    loss_types = [
                        "total_loss", "kl_loss", "reconstruction_loss",
                        "confidence loss","angle loss","r loss","t loss"
                    ]
                elif opt.arch == "gan":
                    loss = [
