...
def main():
    ...
    for i, grasps_list in enumerate(grasps_lists):
        ...
        # if inst_name == 'mug':
        #     grasps_path_intsance = os.path.join(grasps_path, '0_unit_025_mug', 'sift')
        # if inst_name == 'knife':
        #     grasps_path_intsance = os.path.join(grasps_path, '2_unit_knife_a1', 'sift')
            ...
            for each_cam_points in tqdm(os.listdir(os.path.join(save_root, inst_name, ply_file))):
                if each_cam_points.endswith('npz'):
                    ...
                    for k, grasp_name in enumerate(grasps_file_list):
                        ...
                        # input_rxyzw = quat_mul_tensor(quat_conjugate(trans_2_quat_gpu(R_tran, device='cuda')), hand_r_rxyzw)
                        # new_t_ = quat_apply(quat_conjugate(trans_2_quat_gpu(R_tran, device='cuda')), hand_t)
                        # input_rwxyz = torch.zeros(4).cuda()
                        # input_rwxyz[1:] = r.reshape(4)[:3]
                        # input_rwxyz[0] = r.reshape(4)[3]




                        # input_rxyzw = torch.zeros(4).cuda()
                        # input_rxyzw[:3] = hand_r.clone().reshape(4)[1:]
                        # input_rxyzw[3] = hand_r.clone().reshape(4)[0]
                        # new_t = torch.matmul(hand_t.unsqueeze(0), cam_rr.clone()) + torch.from_numpy(T).cuda().unsqueeze(0)*1000.0
                        ...
                        with open(grasp_name[:-4]+'.pkl', 'rb') as filename:
                            ...
                            if os.path.exists(save_func_root+'.npz'):
                                ...
                                # np.savez(save_func_root, pc=point_camera_corrds, cam_r=R, cam_t=T,
                                #      hand_pc=camera_hand_mesh, hand_key=camera_key_mesh, hand_r=rs, hand_t=t, hand_a=a)
