...
class Shadowhand_FK(object):
    ...
    """
    def __init__(self, npy_dir='/home/hjl/Functional_Graspnet/datasets/utils'):
...
# # ↓ for test
def fk_run(outputs_r, outputs_t, outputs_a,device, ass_idx=(0,1,2,3)):
    # # 输入正向运动学层
    # 3 + 4
    outputs_base = torch.cat((outputs_t, outputs_r), 1)
    # 17(18) -> 27
    ...
    angle_2_pair = torch.ones([2, outputs_a.shape[0]]).to(device)#.cuda()
    angle_1_pair = torch.zeros([2, outputs_a.shape[0]]).to(device)#.cuda()
    ...
    outputs_rotation[:, 21:26] = outputs_a[:, 13:]  # all # outputs_rotation[:, 26]是0
    fk = Shadowhand_FK()
