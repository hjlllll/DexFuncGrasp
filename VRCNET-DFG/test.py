...
def test():
    ...
    dataloader_test = torch.utils.data.DataLoader(dataset_test, batch_size=args.batch_size, shuffle=True, num_workers=int(args.workers))
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader_test):
            ...
            label, inputs_cpu, gt_cpu, file = data
            ...
            if args.save_vis:
                for j in range(args.batch_size):
                    complete = result_dict['out2'][j].cpu().numpy()
                    partial = inputs_cpu[j].cpu().numpy()
                    import trimesh
                    obj = trimesh.PointCloud(partial, colors=[212, 106, 126, 255])
                    full = trimesh.PointCloud(complete, colors=[99, 157, 217, 255])
                    trimesh.Scene([obj, full]).show()
                    save_path = file[j].replace('render_pc_for_completion', 'results')
                    if not os.path.exists(save_path[:-len(save_path.split('/')[-1])]):
                        os.makedirs(save_path[:-len(save_path.split('/')[-1])])
                    np.save(save_path, complete)
