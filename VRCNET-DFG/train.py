...
def train():
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        if epoch % args.epoch_interval_to_save == 0:
            save_model('%s/network.pth' % log_dir, net, optimizer,net_d=net_d, epoch=epoch)
            logging.info("Saving net...")

        if epoch % args.epoch_interval_to_val == 0 or epoch == args.nepoch - 1:
            val(net, epoch, val_loss_meters, dataloader_test, best_epoch_losses, optimizer)


def val(net, curr_epoch_num, val_loss_meters, dataloader_test, best_epoch_losses, optimizer):
    ...
    with torch.no_grad():
        ...
        for loss_type, (curr_best_epoch, curr_best_loss) in best_epoch_losses.items():
            if (val_loss_meters[loss_type].avg < curr_best_loss and loss_type != 'f1') or \
                ...
                save_model('%s/best_%s_network.pth' % (log_dir, loss_type), net, optimizer, epoch=curr_epoch_num)
...
if __name__ == "__main__":
    ...
    if args.load_model:
        ...
        exp_name = args.model_name + '_' + args.loss + '_' + args.flag + '_' + time[:10]
